...
class App extends Component {
  ...
  summaryText = () => {
    return (
      <div className="summaryText">
        <h2>
          <FormattedMessage id="summary.headline" />
        </h2>
        {Object.keys(this.state.analysis).map((analysis) => (
          <div
            className="div-analysis"
            key={this.state.analysis[analysis].headline}
          >
            <div>
              <h5
                style={{ backgroundColor: this.state.analysis[analysis].color }}
              >
    ...
    return (
      <svg viewBox="0 0 400 400">
        <VictoryPie
          ...
          height={400}
          colorScale={[
            this.state.data.types.red.color,
  ...
  onSubmit = (e) => {
    ...
    var chartData = [
      ...
      {
        y: numGreen,
        label: ((numGreen / totalQuestions) * 100).toFixed(2) + "%",
      },
      {
        y: numBlue,
        label: ((numBlue / totalQuestions) * 100).toFixed(2) + "%",
      },
      {
        y: numYellow,
        label: ((numYellow / totalQuestions) * 100).toFixed(2) + "%",
      },
    ];
            ...
            <ul>
              {Object.keys(data.answers).map((answer, i) => (
                <li key={i}>
                  <label
                    className={
                      this.state.answers[index] === data.answers[answer].color
                        ? "radio-selected"
                        : ""
                    }
                  >
    ...
    return (
      <div>
        ...
        <button
          ...
          type="button"
          disabled={
            this.state.activeQuestion >= this.state.data.questions.length - 1 ||
            this.state.activeQuestion >= this.state.answers.length
          }
              ...
              {this.state.showSummary === true && (
                <CSSTransition classNames="chart" timeout={300}>
                  <div className="row summaryWrapper">
                    <div className="col-lg-6">{this.summaryText()}</div>
                    <div className="col-lg-6">{this.summaryChart()}</div>
                  </div>
                </CSSTransition>
              )}
            </TransitionGroup>
